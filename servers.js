...
app.get('/', (req, res) => {
    res.send("Valivion Hub Backend v2.0 is Running!");
});

// Пинг от чита (каждые 2.5 сек)
app.post('/api/ping', (req, res) => {
    ...
    // Фильтруем сообщения (общие + сообщения своего сквада)
...
app.post('/api/clone/spawn', (req, res) => {
    ...
    if (userClones.length >= MAX_CLONES_PER_USER) {
        return res.status(400).json({ error: "Limit reached (5 max)" });
    }
...
app.listen(PORT, () => {
    console.log(`[VALIVION] Сервер работает на порту ${PORT}`);
});